...
class GestionUsers():
    ...
    def users_afficher_data(self):
        try:
            ...
            strsql_users_afficher = """SELECT id_user, firstname_user, lastname_user, mail, phone, address, city, npa, user, date_user FROM t_user INNER JOIN t_user ON t_user.fk_user = t_user.id_user ORDER BY id_user ASC"""
 ...
 def add_user_data(self, valeurs_insertion_dictionnaire):
        try:
            ...
            # OM 2020.04.07 C'EST LA QUE VOUS ALLEZ DEVOIR PLACER VOTRE PROPRE LOGIQUE MySql
            strsql_insert_user = """INSERT INTO t_user (id_user,user) VALUES (NULL,%(value_user)s)"""
            # Du fait de l'utilisation des "context managers" on accède au curseur grâce au "with".
    ...
    def edit_user_data(self, valeur_id_dictionnaire):
        try:
            ...
            str_sql_id_user = "SELECT id_user, user FROM t_user WHERE id_user = %(value_id_user)s"
            ...
            print(f"Problème edit_user_data Data Gestions Users numéro de l'erreur : {erreur}")
            # flash(f"Flash. Problèmes Data Gestions Users numéro de l'erreur : {erreur}", "danger")
            # OM 2020.04.09 On dérive "Exception" par le "@obj_mon_application.errorhandler(404)" fichier "run_mon_app.py"
            # Ainsi on peut avoir un message d'erreur personnalisé.
            raise Exception(
                "Raise exception... Problème edit_user_data d'un user Data Gestions Users {erreur}")

    def update_user_data(self, valeur_update_dictionnaire):
        try:
            print(valeur_update_dictionnaire)
            # OM 2019.04.02 Commande MySql pour la MODIFICATION de la valeur "CLAVIOTTEE" dans le champ "nameEditIntituleUserHTML" du form HTML "UsersEdit.html"
            # le "%s" permet d'éviter des injections SQL "simples"
            # <td><input type = "text" name = "nameEditIntituleUserHTML" value="{{ row.intitule_user }}"/></td>
            str_sql_update_user = "UPDATE t_user SET user = %(value_user)s WHERE id_user = %(value_id_user)s"
            ...
            with MaBaseDeDonnee().connexion_bd as mconn_bd:
                ...
                TypeError) as erreur:
            # OM 2020.03.01 Message en cas d'échec du bon déroulement des commandes ci-dessus.
            print(f"Problème update_user_data Data Gestions Users numéro de l'erreur : {erreur}")
            # flash(f"Flash. Problèmes Data Gestions Users numéro de l'erreur : {erreur}", "danger")
            # raise Exception('Raise exception... Problème update_user_data d\'un user Data Gestions Users {}'.format(str(erreur)))
            if erreur.args[0] == 1062:
                ...
                raise Exception("Raise exception... Problème update_user_data d'un user DataGestionsUsers {erreur}")
    ...
    def delete_select_user_data(self, valeur_delete_dictionnaire):
        try:
            ...
            # OM 2020.04.07 C'EST LA QUE VOUS ALLEZ DEVOIR PLACER VOTRE PROPRE LOGIQUE MySql
            # Commande MySql pour afficher le user sélectionné dans le tableau dans le formulaire HTML
            str_sql_select_id_user = "SELECT id_user, user FROM t_user WHERE id_user = %(value_id_user)s"
            ...
            with MaBaseDeDonnee().connexion_bd as mconn_bd:
                ...
                TypeError) as erreur:
            # DEBUG bon marché : Pour afficher un message dans la console.
            print(f"Problème delete_select_user_data Gestions Users numéro de l'erreur : {erreur}")
            # C'est une erreur à signaler à l'utilisateur de cette application WEB.
            flash(f"Flash. Problème delete_select_user_data numéro de l'erreur : {erreur}", "danger")
            raise Exception(
                "Raise exception... Problème delete_select_user_data d\'un user Data Gestions Users {erreur}")

    def delete_user_data(self, valeur_delete_dictionnaire):
        try:
            print(valeur_delete_dictionnaire)
            # OM 2019.04.02 Commande MySql pour EFFACER la valeur sélectionnée par le "bouton" du form HTML "UsersEdit.html"
            # le "%s" permet d'éviter des injections SQL "simples"
            # <td><input type = "text" name = "nameEditIntituleUserHTML" value="{{ row.intitule_user }}"/></td>
            ...
            with MaBaseDeDonnee().connexion_bd as mconn_bd:
                ...
                TypeError) as erreur:
            # DEBUG bon marché : Pour afficher un message dans la console.
            print(f"Problème delete_user_data Data Gestions Users numéro de l'erreur : {erreur}")
            # flash(f"Flash. Problèmes Data Gestions Users numéro de l'erreur : {erreur}", "danger")
            if erreur.args[0] == 1451:
                ...
                # en MySql le moteur INNODB empêche d'effacer un user qui est associé à un film dans la table intermédiaire "t_users_films"
                # il y a une contrainte sur les FK de la table intermédiaire "t_users_films"
                # C'est une erreur à signaler à l'utilisateur de cette application WEB.
                # flash(f"Flash. IMPOSSIBLE d'effacer !!! Ce user est associé à des users dans la t_users_films !!! : {erreur}", "danger")
                # DEBUG bon marché : Pour afficher un message dans la console.
                print(
                    f"IMPOSSIBLE d'effacer !!! Ce user est associé à des films dans la t_users_films !!! : {erreur}")
