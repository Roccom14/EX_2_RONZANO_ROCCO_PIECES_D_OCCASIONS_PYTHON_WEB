...
from APP_FILMS.GENDERS.data_gestion_genders import GestionGenders
...
# ---------------------------------------------------------------------------------------------------
@obj_mon_application.route("/genders_afficher", methods=['GET', 'POST'])
def genders_afficher():
...
# au navigateur par la méthode "render_template".
# On change la valeur d'un gender de films par la commande MySql "UPDATE"
# ---------------------------------------------------------------------------------------------------
...
def genders_delete():
    ...
    if request.method == 'POST':
        try:
            ...
            # Cette erreur 1451, signifie qu'on veut effacer un "gender" de films qui est associé dans "t_genders_films".
            if erreur.args[0] == 1451:
                # C'est une erreur à signaler à l'utilisateur de cette application WEB.
                flash('IMPOSSIBLE d\'effacer !!! Cette valeur est associée à des films !')
                # DEBUG bon marché : Pour afficher un message dans la console.
                print(f"IMPOSSIBLE d'effacer !! Ce gender est associé à des films dans la t_genders_films !!! : {erreur}")
